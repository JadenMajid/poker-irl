...
# 2. Update IRLOptimiser to use entropy_coef
import sys
...
new_irl = """class IRLOptimiser:
    def __init__(
        ...
        self.S              = max(S, 1.0)
        self.entropy_coef   = 0.01  # From base_agent_config.json
    ...
    def current_alpha(self) -> float:
        # PPO shifts logits by R / (S * entropy_coef).
        # We model theta * A_var_n. 
        # So theta_0 / var_norm = -alpha / (S * entropy_coef)
        # alpha = -theta_0 * S * entropy_coef / var_norm
        return float(-self.theta[0].item() * self.S * self.entropy_coef / self.var_norm)

    @property
    def current_beta(self) -> float:
        # theta_1 = beta / (S * entropy_coef)
        return float(self.theta[1].item() * self.S * self.entropy_coef)
